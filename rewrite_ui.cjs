const fs = require('fs');
let code = fs.readFileSync('src/screens/guru/Grades.tsx', 'utf8');

// Replace header
code = code.replace(
  /<div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-50">[\s\S]*?<\/button>\s*<\/div>/,
...
// Replace body
code = code.replace(
  /\{\/\* Tugas Section \*\/\}[\s\S]*?\{\/\* PTS \& PAS Section \*\/\}/,
  `{/* Tugas & UH Section */}
...
fs.writeFileSync('src/screens/guru/Grades.tsx', code);
